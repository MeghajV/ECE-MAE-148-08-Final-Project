...
from utils import dist, arr_to_img, is_number_type
...
class AbstractPath:
    ...
    def run(self, recording, x, y):
        if recording:
            ...
            if d > self.min_dist:
                la = self.determine_look_ahead(x, y)
                if la != -1:
                    if self.lookahead != -1:
                        self.lookahead = min(self.lookahead, la)
                    else:
                        self.lookahead = la
    ...
    def determine_look_ahead(self, x, y):
        for index, (x_path, y_path) in reversed(list(enumerate(self.path))):
            d = dist(x, y, x_path, y_path)
            if d < self.min_dist:
                logging.info(f"Detected Crossover at points ({x}, {y}) and ({x_path}, {y_path})")
                print(f"Detected Crossover at points ({x}, {y}) and ({x_path}, {y_path})")
                look_ahead = len(self.path) - index #index of the new point - the index of the point thats close
                return look_ahead
